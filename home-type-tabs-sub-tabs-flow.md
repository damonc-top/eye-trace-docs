...
| `go build` | 直接 `cp /tmp/et/banner-server /tmp/banner-server`,启动用 env var(避免 inline DSN 触发密码暴露规则) |
...
- **密码内嵌规则**:Go 二进制必须落在 `/tmp/` 类 sandbox 可见路径;DB 密码不能 inline 在命令行 → 用 env var 注入(`MYSQL_PWD` 或 `DB_DSN` 由外部脚本导出)。
